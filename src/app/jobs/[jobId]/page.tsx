...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${job.title} Job - Techmiya EdTech`;
  const description = `${job.title} opening at Techmiya EdTech, Jayanagar Bangalore. ${job.fullDescription.substring(0, 150)}...`;

  return {
    title,
    description,
    alternates: {
      canonical: `https://www.techmiyaedtech.com/jobs/${job.id}`,
    },
    openGraph: {
      title,
      description,
      url: `https://www.techmiyaedtech.com/jobs/${job.id}`,
      type: "website",
    },
  };
}
